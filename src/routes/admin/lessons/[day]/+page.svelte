<script lang="ts">
	...
	$effect(() => {
		...
		title_fa = data.lesson.title_fa ?? "";
		group = data.lesson.group;
...
<svelte:head
	><title>Edit Day {data.lesson.day} - Mirifer Admin</title></svelte:head
>
	...
	{#each sentences as s, i (i)}
		<div class="sentence-card" class:sent={s.role === "sent"}>
			<div class="sentence-meta">
				...
				<button class="btn-remove" onclick={() => removeSentence(i)}
					>✕</button
				>
			</div>
			<div class="sentence-fields">
				...
				<label>
					...
					<input
						type="text"
						bind:value={s.translation_fa}
						dir="rtl"
						placeholder="صبح بخیر!"
					/>
				</label>
			</div>
		</div>
