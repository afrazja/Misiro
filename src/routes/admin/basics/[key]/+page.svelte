<script lang="ts">
	...
	$effect(() => {
		...
		title_en = data.category.title_en;
	...
	function removeSectionWord(si: number, wi: number) {
		sections = sections.map((s, idx) =>
			idx === si
				? { ...s, words: s.words.filter((_, j) => j !== wi) }
				: s,
		);
	}
		...
		try {
			const res = await fetch(`/admin/basics/${data.category.key}`, {
				...
				body: JSON.stringify({
					...
					sections:
						data.category.type === "multi" ? sections : undefined,
				}),
			});
			if (!res.ok) {
...
<svelte:head
	><title>Edit {data.category.title_en} - Mirifer Admin</title></svelte:head
>
				...
				{#each words as w, i (i)}
					<tr>
						...
						<td
							><input
								type="text"
								bind:value={w.example_fa}
								dir="rtl"
								placeholder="خانه بزرگ است."
							/></td
						>
						<td>
							<button class="btn-rm" onclick={() => removeWord(i)}
								>✕</button
							>
						</td>
					</tr>
		...
		<div class="section-card">
				...
				<div class="json-note">
					...
					<label>
						...
						<textarea
							rows="10"
							bind:value={
								() => JSON.stringify(sec.tenses ?? [], null, 2),
								(v) => {
									try {
										sec.tenses = JSON.parse(v);
									} catch {}
								}
							}
						></textarea>
			...
			{:else}
				<div class="section-words-header">
					...
					<button class="btn-add" onclick={() => addSectionWord(si)}
						>+ Add Word</button
					>
				</div>
				<div class="words-table-wrap">
